...
class Lescript
{
    ...
    public function signDomains(array $domains, $reuseCsr = false)
    {
        ...
        foreach ($domains as $domain)
        {
            ...
            if (is_array($response) && is_array($response['challenges']))
            {
        ...
        $csr = $reuseCsr && is_file($domainPath . "/last.csr") ? $this->getCsrContent($domainPath . "/last.csr") :
            $this->generateCSR($privateDomainKey, $domains);

        // request certificates creation
        $result = $this->signedRequest("/acme/new-cert", [
            'resource' => 'new-cert',
            'csr' => $csr
        ]);
